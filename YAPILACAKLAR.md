...
**Durum:** active · 2026-09-24 — F16.1 `tamamlandı`; **F16.2 `işleniyor`** (F16.2.1–F16.2.5, F16.2.8–F16.2.9 `tamamlandı`; F16.2.6–F16.2.7/F16.2.10–F16.2.11 `bekliyor`); F16.3–F16.6 `bekliyor` · docs envanter: `docs/F16.2-durum-ve-eksikler.md`
...
| [EK-20260924] F16.2.4 | Blog: `blog_yazilari` + `blog_yazisi_cevirileri` seed (geliştirici aded/konu onayı) × 6 dil + SEO band + admin CRUD doğrulama + `blog.php` HTML | overmind | geliştirici | DB count + band + API/HTML + rapor | tamamlandı |
| [EK-20260924] F16.2.5 | Şehirler: `sehir_cevirileri` (3 şehir × 6 dil) + SEO + `sehir-landing` içerik; migration `000043` (18+36, idempotent+down); canonical slug fix; dil-karışımı LANG fix; sitemap 36; HTML 36/36 + `/seo/check` key 36/36 + manuel L1+L2+SEO PASS; rapor `docs/F16.2.5-sehir-raporu.md` | overmind | geliştirici | DB count + band + HTML + rapor | tamamlandı |
...
| [EK-20260924] F16.2.8 | Garanti: `ayarlar` `garanti_suresi|garanti_kapsami|garanti_istisnalari` (+ site_adi) doldur; `garanti.php` kanıt | overmind | geliştirici | ayarlar SELECT + HTML + **rapor `docs/F16.2.8-garanti-raporu.md`** | tamamlandı |
| [EK-20260924] F16.2.9 | Rehberler: **mimari karar** — 4 statik `rehber-*.php` koru vs DB’ye al; karar sonrası uygulama | overmind | geliştirici | karar kaydı + HTML/API | tamamlandı |
...
| 2026-09-23 | F16.2.2 3 karar onayı: (1) fiyat = **formül sütunları** — geliştirici tablosunda 9/12 satır hatalıydı (ALT-002, KAR-003, DIK-005, MOD-007, MOD-008, KLA-009, KLA-010, KAR-011, MOD-012); HesapService canlı motoru geçerli, E-E-A-T için içerik↔araç eşitliği; (2) kapasite = DB F15 değerleri (örnekteki 2.5 hatalı); (3) `baslik` 40-70 byte = kullanım adı eklenerek (`[Model] [Malzeme] Kamelya [Ölçü] [Kullanım]`) | F16 | kabul edildi |
...
| 2026-09-23 | F16.2.2 **Grup 1** (6 ürün × 6 dil) migration `2026_09_23_000039_urun_seed.php` uygulandı: DB 12/36/36; band 36/36 + seo 36/36 + slug_dup=0; down/up + idempotent 2. up=0 ✓; public API 200 (6), admin API 200 total=12; HTML 6 dil + 6 ürün TR 200; AdminUrunService DI `use` düzeltmesi (+1 satır); rapor `docs/F16.2.2-urun-raporu.md` yazıldı; öz-denetim PASS → **geliştirici Grup 1 onayı verildi** | F16 | tamamlandı |
...
| 2026-09-24 | **F16.2.8 Garanti tamamlandı:** 3 anahtar (`garanti_suresi|kapsam|istisnalari`, 6-dil JSON) · migration `2026_09_24_000041_garanti_seed.php` (idempotent ON DUPLICATE KEY + down) · up/up(0)/down/up döngüsü PASS · DB 3 satır `JSON_VALID=1` `JSON_KEYS=6` · public API 200 3 anahtar (whitelist F10.1 zaten vardı) · `garanti.php` `$sure` `$coz()` parse fix + `$SEO_HAM` band title 50–60/desc 150–160 6/6 · HTML `/tr\|en\|de\|fr\|it\|ar/garanti` 200 içerik 6/6 · manuel L1+L2 PASS · SEO KISMİ PASS · rapor `docs/F16.2.8-garanti-raporu.md` | F16 | tamamlandı |
| 2026-09-24 | **[EK-20260924] F16.2.9 Rehberler tamamlandı:** V1 mimari karar = 4 statik `rehber-*.php` korunur (DB'ye alma V2) · migration `2026_09_24_000042_rehber_seo_seed.php` → `seo_verileri` 24 satır (4 path × 6 dil, byte band 24/24, hreflang 7, canonical dil bazlı) · up/up(0)/down/up + gocler PASS · frontend `$SEO_HAM` 4 dosya (frontend↔migration 24/24) · router +2 route (fark/istanbul, öncesi 404) + sitemap rehber 12→24 + ozellik null fail-open · lang +21×6 → 94 key · `SeoService::bantKontrol` mb_strlen→strlen (byte) · keşif fix: istanbul `str_pad(int)` fatal (satır 152) · HTML 24/24 PASS · `/seo/check` key 24/24 PASS · manuel L1+L2+SEO PASS · rapor `docs/F16.2.9-rehber-raporu.md` | F16 | tamamlandı |
| 2026-09-24 | **[EK-20260924] F16.2.5 Şehirler tamamlandı:** V1 = 3 şehir × 2 path × 6 dil · migration `2026_09_24_000043_sehir_ceviri_seed.php` → `sehir_cevirileri` 18 + `seo_verileri` 36 (byte band 18/18 + 36/36, canonical slug dahil 36/36, hreflang 7, robots+kw 36) · up/up(0)/down/up + gocler PASS · bölge 5 intact · frontend `sehir-landing.php` `seo_baslik`/`seo_aciklama` + H1 tercihi · sitemap 36 şehir URL (toplam 240) · keşif: canonical slug bug fix + **LANG dil-karışımı fix** (12 HTML fail → 0; 9 ceviri + 14 seo başlığı dil-doğru) + 2× `·` desc temizliği · HTML 36/36 + LANG 0 + `/seo/check` key 36/36 · manuel L1+L2+SEO PASS · rapor `docs/F16.2.5-sehir-raporu.md` | F16 | tamamlandı |
