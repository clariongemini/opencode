...
use Kamelya\Validators\Admin\AdminSssValidator;

/** Admin SSS CRUD — silme = pasifleştirme. */
final class AdminSssController
{
    private AdminSssService $service;

    public function __construct()
    {
        $pdo = Database::baglanti();
        $this->service = new AdminSssService($pdo, new SssYonetimRepository($pdo), new AuditLogService($pdo));
    }
    ...
    public function olustur(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminSssValidator::dogrula($veri);
    ...
    public function guncelle(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminSssValidator::dogrula($veri, true);
            if (!$dogrulama['gecerli']) {
                throw new Hata('VALIDATION_ERROR', 'Doğrulama hatası.', $dogrulama['hatalar'], 422);
            }
