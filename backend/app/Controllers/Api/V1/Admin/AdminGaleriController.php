...
use Kamelya\Services\Admin\AdminGaleriService;
use Kamelya\Services\AuditLogService;
use Kamelya\Validators\Admin\AdminGaleriValidator;

/** Admin galeri CRUD — silme = pasifleştirme. */
final class AdminGaleriController
{
    private AdminGaleriService $service;

    public function __construct()
    {
        $pdo = Database::baglanti();
        $this->service = new AdminGaleriService($pdo, new GaleriYonetimRepository($pdo), new AuditLogService($pdo));
    }
    ...
    public function olustur(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminGaleriValidator::dogrula($veri);
    ...
    public function guncelle(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminGaleriValidator::dogrula($veri, true);
            if (!$dogrulama['gecerli']) {
                throw new Hata('VALIDATION_ERROR', 'Doğrulama hatası.', $dogrulama['hatalar'], 422);
            }
