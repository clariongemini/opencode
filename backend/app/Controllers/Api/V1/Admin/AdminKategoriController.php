...
use Kamelya\Validators\Admin\AdminKategoriValidator;

/** Admin kategori CRUD — silme = pasifleştirme. */
final class AdminKategoriController
{
    private AdminKategoriService $service;

    public function __construct()
    {
        $pdo = Database::baglanti();
        $this->service = new AdminKategoriService($pdo, new KategoriYonetimRepository($pdo), new AuditLogService($pdo));
    }

    /** @param array<string, string> $rota */
    public function liste(Request $istek, array $rota = []): void
    {
        $tur = isset($istek->sorgu['tur']) && is_string($istek->sorgu['tur']) ? $istek->sorgu['tur'] : null;
        Response::basari($this->service->liste($tur));
    }
    ...
    public function olustur(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminKategoriValidator::dogrula($veri);
    ...
    public function guncelle(Request $istek, array $rota = []): void
    {
        try {
            $veri = $istek->govde ?? [];
            $dogrulama = AdminKategoriValidator::dogrula($veri, true);
            if (!$dogrulama['gecerli']) {
                throw new Hata('VALIDATION_ERROR', 'Doğrulama hatası.', $dogrulama['hatalar'], 422);
            }
